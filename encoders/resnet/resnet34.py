...
class ResNetEncoder(ResNetBaseEncoder):
    def __init__(self, in_channels=3, wavelets_mode=False, *args, **kwargs) -> None:
        ...
        self.encoder_block4 = ResNetEncoderBasicBlock(
            in_channels=128 + (3 if wavelets_mode == 2 else 0),
            out_channels=256 + (3 if wavelets_mode == 2 else 0),
            num_blocks=5,
            pool_block=False,
            wavelets_mode=wavelets_mode,
        )
        ...
        self.encoder_block5 = ResNetEncoderBasicBlock(
            in_channels=256 + (4 if wavelets_mode == 2 else 0),
            out_channels=512 + (4 if wavelets_mode == 2 else 0),
            num_blocks=2,
            pool_block=False,
            wavelets_mode=wavelets_mode,
        )
