...
class Vgg13Encoder(VggBaseEncoder):
    def __init__(self, in_channels=3, wavelets_mode=False, *args, **kwargs) -> None:
        ...
        self.encoder_block2 = EncoderBlock(
            64 + (1 if wavelets_mode == 2 else 0), 128, wavelets_mode=wavelets_mode
        )
        self.encoder_block3 = EncoderBlock(
            128 + (1 if wavelets_mode == 2 else 0), 256, wavelets_mode=wavelets_mode
        )
        self.encoder_block4 = EncoderBlock(
            256 + (1 if wavelets_mode == 2 else 0), 512, wavelets_mode=wavelets_mode
        )
        self.encoder_block5 = EncoderBlock(
            512 + (1 if wavelets_mode == 2 else 0), 512, wavelets_mode=wavelets_mode
        )
